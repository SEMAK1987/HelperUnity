...
6. **Результат:** Blender автоматически сге�### 🌑 РЕШЕНИЕ ПРОБЛЕМЫ: ПОЧЕМУ ПЕРСОНАЖ СТАЛ СЛИШКОМ ТЕМНЫМ?

На ваших скриншотах видно, что Маг успешно оделся в текстуры, но его лицо и мантия выглядят затемнёнными. Это абсолютно стандартная ситуация при PBR-настройке, и она легко решается изменением освещения.

Вот подробный разбор, **почему это произошло, на каких скриншотах искать кнопки и как всё настроить**:

---

#### 1️⃣ Причина №1: Неправильное направление Солнца (Directional Light)
На вашем **третьем скриншоте (Inspector источника Directional Light)** мы видим настройки поворота солнца (**Rotation**):
* **X = 40, Y = 180, Z = 0** (ранее было Y = 30)
Вы развернули солнце на `Y = 180`, что отлично! Но если персонаж всё ещё кажется темноватым, отрегулируйте угол высоты солнца (**X**). 
* **Как сделать еще лучше:**
  * Попробуйте установить **X = 30** или **X = 35** (это опустит солнце чуть ближе к горизонту, чтобы оно светило Магу прямо в лицо, красиво подсвечивая бороду и складки одежды, а не светило прямо сверху вниз).

---

#### 2️⃣ Причина №2: Отключено освещение в самом окне Scene (Scene View Lighting)
На вашем **третьем скриншоте (настроек солнца)** в желтой рамке горит предупреждение:
> *"Lighting has been disabled in at least one Scene view. Any changes applied to lights in the Scene will not be updated..."*

Это означает, что в окне редактора **Scene** у вас просто отключена кнопка отображения источников света, поэтому изменения солнца не показываются во вьюпорте в реальном времени.

* **На каком скриншоте это исправить:** 
  На вашем **первом скриншоте (где открыта вкладка Scene и виден серый силуэт Мага вдалеке)**.
* **🛠️ Как исправить:**
  1. Посмотрите на верхнюю панель инструментов прямо внутри окна **Scene** (чуть ниже основных вкладок Unity, там, где расположены иконки инструментов перемещения и вращения объектов).
  2. В левой части этой внутренней панели найдите маленькую иконку **Лампочки (Toggle Scene Lighting)** — на вашем первом скриншоте она сейчас серая и выключенная. Она расположена рядом с иконкой динамика и иконкой неба/облаков.
  3. **Кликните по этой иконке Лампочки**, чтобы она подсветилась синим (активным) цветом.
  4. Теперь Unity начнет честно и красиво отображать свет от Directional Light прямо в окне редактора!

---

#### 3️⃣ Причина №3: Слабый окружающий (рассеянный) свет (Ambient Light)
Если после настройки солнца теневые участки на Маге всё равно кажутся вам слишком черными, нужно немного прибавить общую яркость окружения.
...
* **🛠️ Как исправить:**
  ...
  5. Это мягко подсветит все темные теневые участки персонажа со всех сторон, сделав картинку более сочной и живой!

* **🛠️ Как исправить в Unity 6 (где искать нужную кнопку на панели):**
  На ваших новых скриншотах крупным планом всё стало предельно ясно! На первом скриншоте у вас показаны первые три кнопки панели:
  1. Кнопка **`2D`** (перечеркнутая).
  2. Кнопка **Динамика/Динамик** (перечеркнутый динамик со всплывающей подсказкой *«Toggle audio on or off»*).
  3. **ТРЕТЬЯ кнопка, которая идет сразу же ПОСЛЕ динамика!** На ней нарисован наклонный фонарик/прожектор, который тоже **перечеркнут диагональной чертой**. Справа от него есть маленькая стрелочка-треугольник, направленная вниз.
  
  👉 **Вам нужно нажать именно на эту третью кнопку (на сам перечеркнутый фонарик, левее маленькой стрелочки)!**
  
  *Что произойдет:*
  * Как только вы нажмете на неё, диагональная линия исчезнет, а кнопка с фонариком подсветится приятным синим/светлым цветом.
  * Если вы наведете на неё мышку, всплывет подсказка **«Toggle scene lighting on or off»** (Включить/выключить освещение сцены).
  * Ваш Маг мгновенно озарится ярким, красивым и сочным светом от солнца (Directional Light)!

  *Примечание:* Кнопка с изображением глаза (`Number of hidden objects...`) и кнопка с камерой (`Settings for the Scene view camera`), которые вы открыли на втором и третьем скриншотах — это другие инструменты, их трогать не нужно. Нужная кнопка находится в самом левом блоке, третья по счету (сразу справа от динамика).

* **🏃‍♂️ ЧТО ДЕЛАТЬ ДАЛЬШЕ?**
  Когда вы настроите освещение и ваш Маг станет ярким и красивым, переходите к настройке анимаций:
  1. **Настройка Rig модели Мага:**
     * Выберите в окне **Project** файл исходной 3D-модели Мага: **`Magic_Battles_Osnova`** (файл с иконкой синего кубика).
     * В окне **Inspector** (справа) перейдите на вкладку **Rig** (она находится рядом с вкладками *Model*, *Animation* и *Materials*).
     * Убедитесь, что параметр **Animation Type** установлен в значение **`Humanoid`**.
     * В поле **Avatar Definition** выберите **`Create From This Model`**.
     * Нажмите кнопку **`Apply`** в самом низу инспектора, чтобы сохранить изменения. Unity создаст аватар костей для вашего Мага.

2. **Подключение анимации:**
   * Теперь вы можете перетащить на этого Мага любые готовые humanoid-анимации (например, настроенную по нашему гайду анимацию **`Mage_Idle`**).
   * Персонаж мгновенно оживёт, примет правильную боевую стойку мага и начнет дышать!
...
* **Meshes ➔ Read/Write:** Снимите галочку ❌.
  * *Почему:* Если вы не планируете деформировать или модифицировать сетку программно из C# кода во время игры, отключение этой опции сэкономит до 50% оперативной памяти (RAM) на каждую модель!
* **Optimize Mesh:** Установите в **`Everything`** (максимальная оптимизация вершин для видеокарты).
* **Generate Colliders:** Снимите галочку ❌ (Коллайдеры настраиваются вручную на самом префабе через компонент Capsule Collider).
* **Geometry ➔ Weld Vertices:** Оставьте включенной ✔ (сшивает дублирующиеся вершины).
* **Normals:** Установите в **`Import`** (Импортировать нормали из Blender, чтобы тени и свет лежали мягко).
* Нажмите кнопку **`Apply`** внизу.

#### 3️⃣ Вкладка «ANIMATION» (Анимации)
* **Import Animation:** Снимите галочку ❌!
  * *Почему:* В этих трех файлах находится чистый скин персонажей в Т-позе без анимационных клипов. Отключение импорта анимаций предотвратит создание пустых файлов внутри ассетов и сэкономит вес вашей игры.
* Нажмите кнопку **`Apply`** внизу.

#### 4️⃣ Вкладка «MATERIALS» (Материалы и текстуры)
На ваших скриншотах материалы заблокированы внутри FBX (Use Embedded Materials).
* Чтобы сделать их настраиваемыми:
  * Нажмите кнопку **`Extract Textures...`** (Извлечь текстуры) и выберите или создайте папку (например, `Assets/Models/Characters/Textures/`).
  * Нажмите кнопку **`Extract Materials...`** (Извлечь материалы) и выберите папку (например, `Assets/Models/Characters/Materials/`).
  * *Почему это необходимо:* Встроенные материалы доступны только для чтения. После извлечения они станут обычными редактируемыми файлами материалов Unity. Вы сможете применить к ним шейдер Universal Render Pipeline (`URP/Lit`), настроить неоновое свечение элементов мантии Мага через карту свечения (`HDR Emission`) или добавить карту нормалей (`Normal Map`) для придания металлу доспехов Воина реалистичного объема!
* Нажмите кнопку **`Apply`** внизу.

---

### 🛑 РЕШЕНИЕ ПРОБЛЕМЫ: ПОЧЕМУ КНОПКА «EXTRACT TEXTURES» НЕАКТИВНА ИЛИ НАЖИМАЕТСЯ, НО НИЧЕГО НЕ РАСПАКОВЫВАЕТСЯ?

Если кнопка **`Extract Textures...`** не горит (серая), либо вы нажимаете её, выбираете папку, но **абсолютно ничего не распаковывается**, это происходит по одной простой причине:
> **Внутри файла FBX физически отсутствуют байты картинки.** В Blender текстура может отображаться во вьюпорте, но если в настройках материала Blender она не подключена строго через узел `Image Texture ➔ Base Color`, Blender при экспорте создаёт "пустой" контейнер. Соответственно, Unity пытается сделать экстракт, но извлекать из файла нечего — поэтому папка остаётся пустой, а персонаж — серым или белым.

---

### ⚠️ РЕШЕНИЕ ПРОБЛЕМЫ: ПОЧЕМУ ТЕКСТУРА НАКЛАДЫВАЕТСЯ НЕПРАВИЛЬНО (КРИВО, РАЗМЫТО ИЛИ КУСКАМИ)?

Я внимательно изучил ваш скриншот! Проблема в том, что вы применили в качестве текстуры **не тот файл**.

#### 🔍 В чем разница между вашими файлами:
1. ❌ **Файл `Mar` (Превью-изображение / Портрет):**
   Это плоская 2D-картинка готового Мага в полный рост с прозрачным фоном, которую вы скачали как превью. У неё нет разметки под 3D-модель. Если перетащить её на модель, Unity пытается «натянуть» готовый плоский портрет на сложную 3D-фигуру. В итоге лицо Мага растягивается по всей спине, узоры плывут, и персонаж выглядит хаотично размазанным.
2. ✔ **Настоящая UV-текстура (UV Texture Map / Развертка):**
   Это квадратное изображение, которое выглядит как «разобранный пазл» или «выкройка одежды». На нём лицо, руки, складки ткани и элементы посоха лежат в виде отдельных плоских кусочков. Модель Мага умеет читать только такую специальную развёртку.

---

### 🛠️ ШАГ ЗА ШАГОМ: КАК НАЙТИ И НАЛОЖИТЬ НАСТОЯЩУЮ ТЕКСТУРУ В UNITY

#### Шаг 1. Найдите правильный файл на компьютере (Или «вытащите» его прямо из Blender!)
Вы написали, что делали 3D-модель из картинки (через ИИ-генератор) и импортировали в формате `.obj` без отдельного файла текстуры. 
Но по вашему новому скриншоту из Blender мы видим потрясающую новость: **внутри Blender ваш Маг отображается идеально цветным!** 

Это означает, что файл текстуры (настоящая UV-развертка) **уже находится внутри вашего Blender-проекта**, просто он запечатан (упакован) внутри самого файла. 

Вот два способа мгновенно «достать» этот скрытый файл из Blender:

* **⚡ Способ А: Автоматическая распаковка (Рекомендуется!)**
  1. В Blender в самом верхнем меню нажмите: **File ➔ External Data ➔ Unpack Resources** (Файл ➔ Внешние данные ➔ Распаковать ресурсы).
  2. В появившемся меню (как на вашем скриншоте) выберите второй пункт:
     👉 **`Write files to current directory (overwrite existing files)`** (Записать файлы в текущую директорию с перезаписью).
     
     *Что это сделает:* Blender возьмет все 3 запакованных файла (включая текстуру Мага) и физически запишет их на ваш жесткий диск в ту же папку, где лежит ваш рабочий файл `.blend`.
  3. **Всё!** Blender создаст папку **`textures`** рядом с вашим `.blend` файлом, и там будет лежать настоящая квадратная текстура (например, `texture.png`). Вы её сразу увидите!

* **🎨 Способ Б: Ручное сохранение из вкладки Shading / UV Editing**
  1. В Blender вверху выберите вкладку **UV Editing** или **Shading**.
  2. На одном из экранов (обычно левом, где показываются картинки) вверху по центру нажмите на **иконку картинки с выпадающим списком**.
  3. В списке выберите вашу текстуру (она может называться `BASE COLOR`, `texture` или иметь другое имя вашей модели). Вы сразу увидите квадратную развертку с частями тела Мага.
  4. В меню этого же окна нажмите **Image ➔ Save As...** (Изображение ➔ Сохранить как) и сохраните её на Рабочий стол в формате `.png` или `.jpg`.

#### Шаг 2. Импортируйте вытащенную текстуру в Unity
1. Просто перетащите этот свежесохраненный файл из папки `textures` (или с Рабочего стола) мышкой в окно **Project** в Unity, в папку `Assets/Models/Heroes_Battles/`.
2. В окне Project у вас появится правильная квадратная картинка.

#### Шаг 3. Расставьте текстуры по правильным слотам (PBR Настройка)
Я вижу по вашему новому скриншоту, что вы перетащили все три файла в Unity! Вы большой молодец!
Но сейчас они случайно перепутались местами (из-за чего Маг выглядит как хромированный металлический шар, а цвета на нём нет).

Вот как нужно распределить ваши три файла в инспекторе материала **`Material.001`**:

1.  **Основной цвет (Base Map):**
    *   Возьмите файл **`texture_pbr_20250901`** (это картинка, где видны фиолетовые одежды Мага).
    *   Перетащите её в самый верхний слот **`Base Map`** (сейчас он у вас пустой и белый).

2.  **Карта металличности (Metallic Map):**
    *   Возьмите файл **`texture_pbr_20250901_metallic-texture`** (серо-черная карта отражений).
    *   Перетащите её во второй слот **`Metallic Map`** (сейчас туда ошибочно вставлена цветная текстура).

3.  **Карта рельефа (Normal Map):**
    *   Возьмите файл **`texture_pbr_20250901_normal`** (сине-фиолетовая рельефная текстура).
    *   Перетащите её в третий слот **`Normal Map`** (куда она у вас уже вставлена).
    *   ⚠️ **Самый важный шаг:** Сразу под слотом `Normal Map` появится предупреждение: *«This texture is not marked as a normal map»* (Эта текстура не помечена как карта нормалей).
    *   Нажмите на серую кнопку **`Fix Now`** (Исправить сейчас) прямо под этим слотом! Unity сам настроит файл, и на Маге мгновенно появятся красивые объёмные складки одежды и детали на посохе!

4.  **Регулировка гладкости (Smoothness):**
    *   Слайдер **`Smoothness`** (Гладкость) под картой металличности по умолчанию стоит на `0.5`. 
    *   Если мантии Мага кажутся вам слишком глянцевыми и блестящими (как мокрый пластик), уменьшите это значение до **`0.1`** или **`0.2`**. Это сделает ткань матовой и реалистичной!

5.  **Готово!** Текстура встанет идеально ровно, каждый узор окажется на своём месте, мантия станет фиолетовой, а лицо Мага — прорисованным и аккуратным!
