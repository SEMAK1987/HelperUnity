...
Перед 6. **Результат:** Blender автоматически сгенерирует веса вершин, и теперь при переходе в **Pose Mode** и вращении костей меш будет плавно двигаться вслед за скелетом!

---�ю: **`File` -> `Import` -> `gTF 2.0 (.gltf/.glb)`**.
...
6. **Результат:** Blender автоматически сгенерирует веса влияния костей на вершины меша. Если при привязке произошла ошибка *"Bone Heat Weighting: failed to find solution for one or more bones"*, это означает, что у вашей модели есть перекрещивающиеся полигоны или дублирующиеся вершины. Скрипт выше автоматически решает эту проблему за вас, очищая геометрию методом Merge By Distance! Если вы делаете это вручную:
...
#### ⚙️ ШАГ 5: Экспорт из Blender и Пошаговая настройка в Unity 6 (Для Воина, Стрелка и Мага)

### 📤 1. Как правильно выделять объекты в Blender перед экспортом?
**Ответ на ваш вопрос по скриншотам:** Да, вам необходимо выделить **ОБА** объекта — и скелет (**Armature**), и саму 3D-модель персонажа (**Mesh**). 

**Как это сделать просто:**
1. В окне 3D-вьюпорта зажмите клавишу `Shift` и кликните левой кнопкой мыши сначала по скелету `FateHumanoid_Armature` (чтобы кости подсветились оранжевым), а затем по мешу `node_0`.
2. Или в окне иерархии (Scene Collection) справа выделите оба объекта, зажав `Ctrl`.
3. Нажмите **File ➔ Export ➔ FBX (.fbx)**.
4. В панели настроек экспорта справа настройте следующие параметры:
   * **Limit to:** Поставьте галочку **`Selected Objects`** (Экспортировать только выделенное).
   * **Object Types:** Кликните на **`Armature`**, затем зажмите `Shift` и кликните на **`Mesh`** (так, чтобы подсветились синим только эти два типа, а Camera и Lamp остались серыми).
   * **Transform ➔ Scale:** `1.00`, **Apply Scalings:** `FBX All`.
   * **Armature ➔ Only Deform Bones:** Обязательно включите галочку ✔ (это исключит служебные кости-контроллеры, облегчив модель).
   * **Armature ➔ Add Leaf Bones:** Снимите галочку ❌ (Unity не нужны лишние "концевые" кости на пальцах и голове).
   * **Animation:** Снимите галочку ❌ с блока анимаций (в файлах скинов анимаций быть не должно).

---

### 🛠️ 2. Пошаговая настройка FBX в Unity 6 (Для Воина, Стрелка и Мага)
После того как вы импортировали готовые файлы `Voin_Battles_Osnova.fbx`, `Strelok_Battles_Osnova.fbx` и `Magic_Battles_Osnova.fbx` в Unity, выделите их поочередно в окне **Project** и настройте инспектор справа:

#### 1️⃣ Вкладка «RIG» (Настройки скелета) — Самое важное!
* **Animation Type:** Измените с *Generic* на **`Humanoid`** (Гуманоид).
  * *Почему это важно:* Это активирует мощную систему Mecanim. Воин, Стрелок и Маг смогут без проблем делить между собой общий набор анимаций (покой, бег, атаки из Mixamo). Вам не придется скачивать уникальные анимации под каждого персонажа отдельно.
* **Avatar Definition:** Установите **`Create From This Model`** (Создать из этой модели).
* **Skin Weights:** Оставьте **`Standard (4 Bones)`** (оптимальный баланс производительности и качества деформаций для мобильных платформ и ПК).
* **Strip Bones:** Оставьте галочку включенной ✔ (удаляет неиспользуемые кости для оптимизации памяти).
* Нажмите кнопку **`Apply`** в самом низу.
* **Проверка:** После применения рядом с кнопкой *Configure...* должна загореться зеленая галочка ✔. Это подтверждает, что Unity успешно распознал скелет персонажа!

#### 2️⃣ Вкладка «MODEL» (Геометрия)
* **Scene ➔ Scale Factor:** Оставьте `1.0` (убедитесь, что галочка *Convert Units* активна ✔).
* **Import Cameras:** Снимите галочку ❌ (Камеры из Blender не должны попадать в префаб персонажа в Unity).
* **Import Lights:** Снимите галочку ❌ (Источники света из Blender не нужны внутри префаба).
* **Preserve Hierarchy:** Оставьте выключенным ❌.
* **Meshes ➔ Read/Write:** Снимите галочку ❌.
  * *Почему:* Если вы не пла�### 🎨 ШАГ 1. ГЕНЕРАЦИЯ ОРУЖИЯ В LEONARDO.AI (2D-ИЗОБРАЖЕНИЕ БЕЗ ФОНА)

Для того чтобы нейросеть **Tencent Hunyuan3D** создала качественную 3D-модель, ей нужно подать на вход идеально чистое 2D-изображение оружия: строго сбоку или спереди, на плоском белом/светлом фоне, без теней, бликов и лишних объектов.

#### ⚙️ Общие настройки в Leonardo.ai:
*   **Режим (Model):** Выбирайте **Leonardo Diffusion XL** или **AlbedoBase XL** (они выдают отличную резкость).
*   **Параметр Aspect Ratio (Соотношение сторон):** `1:1` (Квадратное изображение).
*   **Негативный промпт (Negative Prompt):** `out of frame, overlapping, bad quality, shadows, ground shadows, realistic background, hand holding, multiple views, character, person, noise, bad anatomy, blur, glow effects`.

#### 📝 Промпты для генерации оружия (Скопируйте их):

##### 1. ⚔️ Оружие для Воина (Массивный двуручный меч):
> **Prompt:** `A majestic fantasy knight broadsword, straight blade, ornate golden crossguard, dark leather wrapped grip, detailed pommel, isolated on a solid plain white background, flat 2D game asset icon, front view, orthographic projection, no shadows, sharp details, high contrast`

##### 2. 🏹 Оружие для Стрелка (Эльфийский композитный лук):
> **Prompt:** `A high-elf fantasy recurve bow, made of polished ash wood and silver engravings, wrapped leather grip, green silk bowstring, isolated on a solid plain white background, flat 2D game asset icon, side profile view, orthographic projection, no shadows, sharp details, high contrast`

###### ⚔️ ЭТАПЫ СОЗДАНИЯ И ПРИКРЕПЛЕНИЯ ОРУЖИЯ ДЛЯ СТРЕЛКА, МАГА И ВОИНА (ОТ ЛЕОНАРДО ДО UNITY 6)

Поздравляю! Ваши 3D-модели основных персонажей теперь полностью цветные и потрясающе смотрятся на арене! Теперь пришло время вооружить их уникальным 3D-оружием, соответствующим их классам (Меч, Лук, Посох), и настроить правильные боевые анимации с Mixamo.

Ниже приведено подробнейшее пошаговое руководство по созданию оружия, его подготовке и импорту.

---

### 🎨 ЭТАП 1: ГЕНЕРАЦИЯ ОРУЖИЯ В LEONARDO.AI И ИЗОЛЯЦИЯ ОДНОГО ЛУКА СТРЕЛКА

Для того чтобы нейросеть **Tencent Hunyuan3D** создала качественную 3D-модель, ей нужно подать на вход идеально чистое 2D-изображение оружия: строго сбоку или спереди, на плоском белом/нейтральном фоне, без теней, бликов и лишних объектов.

#### ⚠️ РЕШЕНИЕ ПРОБЛЕМЫ ДВОЙНОГО Л�### ⚔️ ЭТАПЫ СОЗДАНИЯ И ПРИКРЕПЛЕНИЯ ОРУЖИЯ ДЛЯ СТРЕЛКА, МАГА И ВОИНА (ОТ ЛЕОНАРДО ДО UNITY 6)
...
#### 🎨 ЭТАП 1: ГЕНЕРАЦИЯ ОРУЖИЯ И ПЕРСОНАЖА ДЛЯ СТРЕЛКА В LEONARDO.AI

Для того чтобы нейросеть **Tencent Hunyuan3D** создала качественную 3D-модель, ей нужно подать на вход идеально чистое 2D-изображение оружия: строго сбоку или спереди, на плоском белом/нейтральном фоне, без теней, бликов и лишних объектов.

##### 📌 Промпты для Генерации Персонажа и Оружия Стрелка (Strelok):

1. **Промпт на Персонажа Стрелка (Strelok) в Leonardo.ai:**
   > **Prompt:** `A high-fidelity 3D style high-elf fantasy archer, full body portrait, holding a bow, wearing elegant green and silver light leather armor, flat plain neutral background, side-profile and front view guidance, sharp game asset detail, 8k resolution`

2. **Промпт на Оружие Стрелка (Эльфийский композитный лук):**
   > **Prompt:** `A high-elf fantasy recurve bow, made of polished ash wood and silver engravings, wrapped leather grip, green silk bowstring, isolated on a solid plain white background, flat 2D game asset icon, side profile view, orthographic projection, no shadows, sharp details, high contrast`
   * **Негативный промпт (Negative Prompt):** `multiple views, duplicate objects, overlapping, out of frame, shadows, realistic background, hand holding, character, person, blur`

##### ⚠️ КРИТИЧЕСКИЙ ЭТАП: ИЗОЛЯЦИЯ ОДНОГО ЛУКА СТРЕЛКА (ПОДГОТОВКА КАРТИНКИ):
При генерации лука в Leonardo.ai нейросеть часто рисует **два варианта лука** на одном холсте (например, тонкий вид «в профиль» слева и красивый изогнутый деревянный лук справа). 
Если загрузить изображение с двумя объектами в Tencent Hunyuan3D без изменений, то нейросеть запутается, попытается объединить их в одну невнятную сдвоенную 3D-модель или выдаст ошибку! **Нам нужен только один лук!**

* **Пошаговая обрезка картинки:**
  1. Откройте ваше изображение с луком в любом стандартном графическом редакторе (Paint, Photoshop, GIMP, Paint.NET или в стандартном инструменте кадрирования на вашем устройстве).
  2. Используйте инструмент **Обрезка (Crop)**, чтобы полностью отрезать левую часть (прямой тонкий вид).
  3. Оставьте на картинке **только правый красивый изогнутый лук** с тетивой, разместив его по центру холста.
  4. Убедитесь, что фон вокруг оставшегося лука остался ровным и серым/белым.
  5. Сохраните результат как `Strelok_Bow_Clean.png`. Теперь на картинке красуется ровно **один идеальный лук**, готовый к генерации 3D!

---

#### 🌀 ЭТАП 2: Создание 3D в Tencent Hunyuan3D

**Tencent Hunyuan3D** — это передовая нейросеть для генерации 3D-моделей по одной картинке (Image-to-3D). Она идеально подходит для создания игрового оружия.

1. Перейдите на официальный сайт: [https://3d.hunyuan.tencent.com](https://3d.hunyuan.tencent.com) (или используйте их официальный Space на Hugging Face).
2. Загрузите ваше подготовленное и очищенное изображение оружия на белом/нейтральном фоне (для Стрелка — ваш обрезанный файл `Strelok_Bow_Clean.png` с **одним** луком!).
3. Нажмите кнопку **Generate** (генерация займет около 1–2 минут).
4. Проверьте модель во встроенном 3D-вьювере на сайте со всех сторон.
5. Нажмите кнопку **Download** (Скачать) и выберите формат **`.gltf`** или **`.glb`** (этот формат лучше всего сохраняет материалы, цвета и текстуры).

---

#### 📐 ЭТАП 3: Настройка Pivot Point в Blender

Перед экспортом в Unity нужно переместить опорную точку вращения (Pivot Point) модели точно в то место рукоятки, за которое персонаж будет её держать, чтобы оружие не "левитировало" в воздухе:

1. Импортируйте модель оружия в Blender: `File ➔ Import ➔ glTF 2.0 (.gltf/.glb)`.
2. Перейдите в режим редактирования (выделите оружие и нажмите клавишу **`Tab`** ➔ **`Edit Mode`**).
3. Выделите всю модель оружия (нажмите клавишу **`A`**).
4. Переместите (`G`), поверните (`R`) и масштабируйте (`S`) модель так, чтобы **рукоятка оружия (то место по центру лука, меча или посоха, за которое персонаж будет держать его ладонью) оказалась строго в центре координат сцены — в точке `(0,0,0)`**.
   * *Зачем это делать:* Точка `(0,0,0)` станет опорным центром (Pivot Point) вашего оружия. При импорте в Unity оно сядет в руку персонажа идеально ровно.
5. Вернитесь в объектный режим (нажмите **`Tab`** ➔ **`Object Mode`**).
6. Нажмите комбинацию клавиш **`Ctrl + A`** и выберите пункт **`Apply All Transforms`** (Применить все трансформации). Теперь центр объекта зафиксирован точно в рукоятке.
7. Экспортируйте файл в формат FBX: `File ➔ Export ➔ FBX (.fbx)`. Назовите файлы понятно: `Weapon_Sword.fbx`, `Weapon_Bow.fbx`, `Weapon_Staff.fbx`.

---

#### 🔗 ЭТАП 4: Сборка в Unity 6 через Sockets (Самый гибкий игровой способ)

Этот способ рекомендуется для игр, так как он позволяет динамически менять оружие у персонажей во время геймплея (например, при покупке нового снаряжения в лавке замка или смене экипировки из инвентаря).

1. Перенесите ваши готовые `.fbx` файлы оружия в проект Unity в окно **Project** (например, в папку `Assets/Models/Heroes_Battles/`).
2. Откройте Prefab или модель вашего персонажа на сцене в Unity (например, Воина `Voin_Battles_Osnova`).
3. В окне **Hierarchy** раскройте иерархию костей Воина до самой ладони:
   * `Voin_Battles_Osnova` ➔ `Root` ➔ `Hips` ➔ `Spine` ➔ `Spine1` ➔ `Spine2` ➔ `RightShoulder` ➔ `RightArm` ➔ `RightForeArm` ➔ **`RightHand`** (Правая рука).
4. Кликните правой кнопкой мыши по кости **`RightHand`** ➔ выберите **`Create Empty`** (Создать пустой объект) и назовите его **`Sword_Socket`**.
5. Поместите 3D-модель меча `Weapon_Sword` внутрь созданного `Sword_Socket` (сделав дочерним объектом в иерархии). Сбросьте координаты меча в **`(0,0,0)`** в инспекторе.
6. Аккуратно отрегулируйте поворот и положение сокета, чтобы рукоятка идеально легла в ладонь.
7. **Для Стрелка (`Strelok_Battles_Osnova`):**
   * Раскройте иерархию костей до левой ладони: `Root` ➔ `Hips` ➔ `Spine` ➔ ... ➔ `LeftShoulder` ➔ `LeftArm` ➔ `LeftForeArm` ➔ **`LeftHand`** (Левая ладонь).
   * Кликните правой кнопкой мыши по кости **`LeftHand`** ➔ выберите **`Create Empty`** и назовите его **`Bow_Socket`**.
   * Поместите 3D-модель лука `Weapon_Bow` внутрь `Bow_Socket`, сбросьте координаты в **`(0,0,0)`** и аккуратно отрегулируйте поворот сокета, чтобы Стрелок держал лук за рукоятку левой рукой, а его **правая рука оставалась полностью свободной для натягивания тетивы со стрелой!**
8. **Для Мага (`Magic_Battles_Osnova`):**
   * Раскройте кости правой руки до ладони (`RightHand`), сделайте там сокет **`Staff_Socket`** и вложите в него мистический посох.

---

#### 🏃 ЭТАП 5: Скачивание правильных боевых анимаций с Mixamo

Обычные анимации бега и ходьбы без оружия приведут к тому, что руки персонажей будут неестественно проходить сквозь 3D-модели меча, лука или посоха. Используйте в поиске Mixamo специализированные запросы:

* **Для Воина (Warrior - Меч):** Ищите по словам:
  * `sword idle` — Боевая стойка готовности с мечом в правой руке.
  * `sword attack` / `sword slash` — Красивые удары и взмахи мечом (сверху, сбоку, круговые).
  * `sword run` / `sword walk` — Бег и ходьба в боевой стойке с мечом.
* **Для Стрелка (Archer - Лук):** Ищите по словам:
  * `standing draw bow` / `standing draw arrow` — Доставание стрелы, натяжение тетивы лука и выстрел.
  * `bow idle` — Стойка ожидания Стрелка с луком в руке.
  * `bow run` / `bow walk` — Бег/ходьба с луком наготове.
* **Для Мага (Mage - Посох):** Ищите по словам:
  * `standing spell cast` / `magic spell` — Применение заклинания с эффектным взмахом посоха вперед.
  * `staff idle` — Величественная опора на посох в режиме покоя.
  * `staff run` — Бег с удерживанием посоха в руке.

---

#### 📥 Параметры скачивания в Mixamo (ВАЖНО - Подробный разбор по вашим скриншотам!):

Когда вы выбрали нужную анимацию, нажмите кнопку **`Download`** в правом верхнем углу. Настройте параметры скачивания в соответствии с вашими скриншотами:

1. **Format (Формат):** Выберите **`FBX Binary (.fbx)`** *(как показано на вашем Скриншоте 1)* или **`FBX for Unity (.fbx)`**.
   * *Разница и выбор:* Обе настройки отлично работают в Unity 6. Однако стандартный `FBX Binary (.fbx)` является наиболее стабильным, универсальным и проверенным бинарным форматом, гарантирующим безупречный импорт костей в современные версии Unity без конфликтов версий.
2. **Skin (Оболочка):** Обязательно переключите на **`Without Skin`** *(как показано на вашем Скриншоте 3)*.
   * *Зачем:* Поскольку тяжелый меш вашего персонажа со всеми текстурами и раскраской уже загружен в Unity, файлы анимаций должны весить очень мало и содержать исключительно информацию о движениях костей («чистый скелет»). Это экономит до 90% веса файлов и бережет оперативную память игры!
3. **Frames per Second (Кадры в секунду):** Выберите **`30`** *(как показано на вашем Скриншоте 2)*.
   * *Зачем:* 30 FPS — это золотой стандарт игровой индустрии, обеспечивающий идеальный баланс между потрясающей плавностью движений персонажа и великолепной производительностью игры на любых устройствах. Для ПК-версий можно выбрать `60`.
4. **Keyframe Reduction (Сокращение ключей):** Установите **`None`** / **`none`** *(как показано на вашем Скриншоте 4)*.
   * *Зачем:* Предотвращает сжатие анимации нейросетью Mixamo. Выбор `None` гарантирует, что движения рук, пальцев и ног персонажа останутся максимально детализированными, плавными, реалистичными и без каких-либо рывков.

Нажмите **`Download`** и сохраните файл. Рекомендуется сразу переименовать файлы на компьютере (например, `Voin_Sword_Idle.fbx`, `Strelok_Bow_Attack.fbx`, `Mage_Staff_Run.fbx`), чтобы не запутаться!

---

#### 🎬 ЭТАП 6: Окончательный запуск в Unity

1. Перенесите скачанные файлы анимаций (`Without Skin`) в папку `Assets/Models/Heroes_Battles/Animations/` в Unity.
2. Для каждого файла анимации настройте вкладку **`Rig`** в инспекторе Unity:
   * **Animation Type:** `Humanoid`
   * **Avatar Definition:** `Copy From Other Avatar`
   * **Source:** Выберите аватар соответствующего персонажа (например, `Voin_Battles_Osnova-Avatar` для анимаций воина).
   * Нажмите **`Apply`**.
3. Настройте вкладку **`Animation`**:
   * Для Idle, Run, Walk поставьте галочку **`Loop Time`** и **`Loop Pose`**.
   * Для Attack, Get Hit, Death галочку **`Loop Time`** ставить **НЕ НУЖНО**.
   * **Запекание в позу (Root Transform) — Спасает от сдвигов и скольжения:** В разделах **Root Transform Rotation**, **Root Transform Position (Y)** и **Root Transform Position (XZ)** обязательно включите галочки **`Bake Into Pose`**, а в выпадающих списках **Based Upon** выберите **`Original`**. Это зафиксирует логические координаты персонажа, исключив скольжение ("эффект дрифта") ног по земле и непроизвольные вылеты/повороты модели в воздух при атаках.
   * Нажмите **`Apply`**.
4. В окне **Project** нажмите правой кнопкой мыши ➔ `Create ➔ Animator Controller`. Назовите его, например, `Warrior_Controller` или `Archer_Controller`.
5. Откройте его двойным кликом (откроется окно Animator). Перетащите туда синие треугольники анимаций покоя (`Voin_Sword_Idle`), бега (`Voin_Sword_Run`) и атаки (`Voin_Sword_Attack`).
6. Создайте переходы (Transitions) между ними на основе параметров типа `Float` (`Speed`) или `Trigger` (`Attack`).
7. Выделите вашего персонажа на сцене и перетащите созданный контроллер в слот **Controller** компонента **Animator**.
8. Запустите игру! Ваш воин будет плавно дышать в боевой стойке с мечом в руке, стрелок натянет лук, а маг эффектно взмахнет светящимся посохом при сотворении заклинания!

---

*Все настройки полностью синхронизированы с текущей версией проекта v18.12.07 и готовы к воспроизведению на вашей боевой арене BattleScene!*

---

*Все настройки полностью проверены для Unity 6 и готовы к работе! Ваш проект «Fate Continent» готов к внедрению боевых анимаций!*айлов) и настройте плавные переходы (Transitions) между стойкой покоя (`Idle`), бегом (`Run`) и атакой (`Attack`) на основе параметров скорости (`Speed`) или триггеров (`AttackTrigger`).
6.  Назначьте готовые контроллеры в компонент **Animator** соответствующих персонажей на сцене! Запустите игру и наслаждайтесь потрясающими боевыми сценами!

---

*Все инструкции полностью синхронизированы с обновленной базой знаний v18.12.07 проекта Fate Continent и полностью готовы к реализации на вашей боевой арене BattleScene!*

---

*Все настройки полностью проверены для Unity 6 и готовы к работе! Ваш проект «Fate Continent» готов к внедрению боевых анимаций!*

---

#### 🛠️ Шаг 2. Разблокируйте материалы импортированных персонажей
Поскольку по умолчанию все материалы импортированных из FBX моделей в Unity защищены от редактирования, нам нужно извлечь их для настройки текстур:
1. Выделите в окне **Project** файл вашей модели (например, `Voin_Battles_Osnova`).
2. В окне **Inspector** справа перейдите на вкладку **Materials**.
3. Нажмите кнопку **`Extract Textures...`** и укажите папку для сохранения текстур (например, `Assets/Models/Heroes_Battles/Textures_Voin/`).
4. Нажмите кнопку **`Extract Materials...`** и укажите папку (например, `Assets/Models/Heroes_Battles/Materials_Voin/`).
5. Нажмите кнопку **`Apply`** в самом низу инспектора.
6. **Повторите эти же действия** для файла **`Strelok_Battles_Osnova`**, выбрав соответствующие папки.
   * *Результат:* Теперь у вас в проекте появились редактируемые файлы материалов для Воина и Стрелка! Они больше не заблокированы.

#### 🛠️ Шаг 3. Импортируйте и настройте текстуры в материалах

##### ⚔️ НАСТРОЙКА ДЛЯ ВОИНА (Voin_Battles_Osnova):
1. Перетащите распакованные из Blender PNG-файлы Воина в вашу папку проекта Unity.
2. В окне **Project** найдите извлеченный материал Воина (например, `Material_Voin` или `Material.002`).
3. В окне **Inspector** настройте слоты материала:
   * **Base Map (Albedo):** Перетащите цветную текстуру Воина (где видны элементы его серебряно-белых доспехов и красного/белого плаща).
   * **Metallic Map:** Если есть текстура с приставкой `_metallic`, перетащите её сюда.
     * *Совет по металлу:* Для доспехов Воина настройте слайдер **`Smoothness`** (Гладкость). Поставьте его примерно на **`0.5` – `0.6`**, чтобы металлические пластины лат красиво и реалистично отражали свет и блестели на солнце!
   * **Normal Map:** Перетащите сине-фиолетовую карту нормалей (если она сгенерирована). Нажмите появившуюся кнопку **`Fix Now`**, чтобы придать латам и плащу рельефность.

##### 🏹 НАСТРОЙКА ДЛЯ СТРЕЛКА (Strelok_Battles_Osnova):
1. Перетащите распакованные из Blender PNG-файлы Стрелка в проект Unity.
2. Найдите извлеченный материал Стрелка (например, `Material_Strelok` или `Material.003`).
3. В окне **Inspector** настройте слоты материала:
   * **Base Map (Albedo):** Перетащите цветную текстуру Стрелка (где виден капюшон, кожаный доспех и колчан со стрелами).
   * **Metallic Map:** Оставьте пустым или перетащите карту металличности (если есть).
     * *Совет по коже и ткани:* Кожаный доспех и ткань не должны блестеть как металл. Установите слайдер **`Smoothness`** (Гладкость) на низкое значение: **`0.1` – `0.2`**. Это сделает его куртку матовой и естественной!
   * **Normal Map:** Перетащите сине-фиолетовую карту нормалей Стрелка и нажмите кнопку **`Fix Now`**. Это проявит каждую деталь на его луке, ремнях портупеи и складках капюшона!

После этого ваши персонажи мгновенно окрасятся в свои законные цвета прямо на арене, встав в один ряд с прекрасно настроенным Магом!

---

### 🌑 РЕШЕНИЕ ПРОБЛЕМЫ: ПОЧЕМУ ПЕРСОНАЖ СТАЛ СЛИШКОМ ТЕМНЫМ?

На ваших скриншотах видно, что Маг успешно оделся в цветную текстуру, но его лицо и одежда выглядят темными (в тени). Это абсолютно стандартная ситуация при PBR-настройке, и она легко решается изменением освещения.

Вот подробный разбор причин и **3 простых шага**, чтобы сделать Мага ярким, сочным и отлично освещенным спереди:

#### 1️⃣ Причина №1: Направление Солнца (Directional Light)
На ваших скриншотах настроек солнца у источника **Directional Light** изначально стоял поворот (**Rotation Y = 30**). Из-за этого солнце светило Магу **сзади-сверху-справа**. Так как Маг стоит лицом к камере, вся его передняя часть (лицо, борода, грудь) находилась в глубокой собственной тени!

* **Как исправить:**
  1. Выберите **Directional Light** в окне Hierarchy (как на вашем третьем скриншоте).
  2. В окне Inspector (справа) в компоненте Transform измените параметры **Rotation** (Поворот):
     * Поставьте **Y = 180** (вы уже сделали это на последнем скриншоте! Теперь солнце светит Магу прямо в лицо!).
     * Поставьте **X = 40** (угол наклона солнца сверху вниз).
  3. Направление света настроено идеально! Теперь солнце светит спереди, красиво подчеркивая бороду и одеяния.

#### 2️⃣ Причина №2: Отключено освещение в самом окне Scene (Scene View Lighting)
На вашем скриншоте настроек солнца в желтой рамке горит предупреждение:
* *"Lighting has been disabled in at least one Scene view. Any changes applied to lights in the Scene will not be updated..."*
Это значит, что в окне редактора Scene у вас просто отключена кнопка отображения источников света, поэтому изменения солнца не показываются в реальном времени.

* **На каком скриншоте нужно включить свет?**
  Вам нужно включить его в **Окне Scene (Сцена) — это ваш Первый скриншот** (где Маг стоит далеко на серой сетчатой арене).
  
* **🛠️ Как исправить в Unity 6 (где нет привычной лампочки):**
  В Unity 6 разработчики изменили дизайн этой кнопки! Теперь она выглядит не как обычная круглая лампочка, а как **наклонный ФОНАРИК (прожектор), который сейчас ПЕРЕЧЕРКНУТ диагональной линией** (это значит, что свет в окне Scene выключен).

  Вот где он находится на вашем скриншоте панели инструментов (которую вы прислали крупным планом):
  1. Посмотрите на горизонтальную панель вверху окна Scene.
  2. Найдите кнопку **`2D`** (она перечеркнута).
  3. **Сразу справа от кнопки `2D`** находится иконка **наклонного серого фонарика (прожектора) с перечеркнутой линией**.
  4. Кликните по этому **перечеркнутому фонарику**! Перечеркивающая линия исчезнет, а сама иконка загорится синим (активным) цветом.
  
  *Примечание:* Справа от него находится ещё один такой же фонарик с маленьким треугольником-стрелочкой внизу (это настройки теней и параметров света, его трогать не нужно — кликайте именно по первому фонарику без стрелочки).

  Как только вы нажмете на этот фонарик, Unity мгновенно включит отображение солнца от Directional Light прямо в окне сцены, и ваш Маг озарится ярким, красивым и сочным светом!

#### 3️⃣ Причина №3: Слабый окружающий (рассеянный) свет (Ambient Light)
Если после настройки солнца тени на Маге с обратной стороны всё равно кажутся вам слишком черными, нужно немного прибавить общую яркость окружения.

* **🛠️ Как исправить:**
  1. В самом верхнем меню Unity откройте: **Window ➔ Rendering ➔ Lighting** (Окно ➔ Рендеринг ➔ Освещение). Откроется специальная вкладка настроек света.
  2. Внутри этого окна перейдите на вкладку **Environment** (Окружающая среда).
  3. Найдите параметр **Environment Lighting ➔ Intensity Multiplier** (или Ambient Intensity). По умолчанию он равен 1.0.
  4. Увеличьте его значение до **1.3** или **1.5**.
  5. Это мягко подсветит все темные теневые участки персонажа со всех сторон, сделав картинку более сочной, мягкой и живой!

---

### 🏃‍♂️ ЧТО ДЕЛАТЬ ДАЛЬШЕ?

Когда вы настроите освещение и ваш Маг станет ярким и красивым, переходите к настройке анимаций:

1. Выберите в окне Project файл модели Мага **`Magic_Battles_Osnova`**.
2. В окне Inspector (справа) перейдите на вкладку **Rig** (рядом с вкладками Model, Materials, Animation).
3. Убедитесь, что **Animation Type** установлен в **`Humanoid`**, а в поле **Avatar Definition** выбрано **`Create From This Model`** (после этого обязательно нажмите кнопку **`Apply`** в самом низу инспектора).
4. Теперь вы можете перетащить на этого Мага любые готовые гуманоидные анимации (например, `Mage_Idle`), которые вы настроили по нашему гайду! Все кости встанут на свои места автоматически!

---

#### 💡 Как предотвратить это при следующем экспорте из Blender?
Если вы всё же хотите, чтобы кнопка *Extract Textures* в Unity работала автоматически:
1. В Blender перейдите во вкладку **Shading** (вверху).
2. Выберите вашего персонажа. В окне редактора нод (внизу) у вас обязательно должна быть нода **`Image Texture`** с загруженной картинкой вашего персонажа, и её выход **`Color`** должен быть соединен с входом **`Base Color`** ноды `Principled BSDF`. Если этой связи нет, Blender считает материал одноцветным и не упаковывает картинку.
3. При экспорте FBX выберите вверху настройки: **Path Mode = `Copy`** и обязательно нажмите на иконку **коробки** (кнопка *Embed Textures*), чтобы она стала синей. Тогда Blender запечатает картинку в FBX, и Unity сможет её извлечь.

Но ручной перенос текстуры в Unity (Способ выше) — это стандартная практика всех профессиональных разработчиков на Unity, так как она даёт полный контроль над текстурами и материалами!

---

### 🏃‍♂️ 3. Настройка Rig у сторонних Анимаций (Файлы анимаций без скина)
...
## 🎭 ИНСТРУКЦИЯ ПО РАБОТЕ С MIXAMO ДЛЯ «FATE CONTINENT»
### Поиск, скачивание, импорт и тонкая настройка анимаций для Мага, Стрелка и Воина

В этой инструкции подробно, по шагам расписано, как найти на Mixamo нужные анимации, скачать их с правильными параметрами, правильно импортировать в Unity 6 и настроить, чтобы персонажи не скользили по земле, двигались плавно и выглядели профессионально!

---

### 🔍 ЭТАП 1: ЧТО И КАК ИСКАТЬ НА MIXAMO (Поисковые запросы)

Перейдите на сайт [mixamo.com](https://www.mixamo.com/) и введите в поле **Search** в левой панели следующие запросы на английском языке (Mixamo понимает только английский).

Вот готовые списки идеальных поисковых фраз для каждого из ваших трех классов:

#### 1️⃣ КЛАСС: МАГ (MAGE / WIZARD)
Маги используют посохи, книги или кастуют заклинания руками. Ищите следующие движения:
*   **Покой (Idle):**
    *   `Mage Idle` (Маг стоит с посохом) — *отличный вариант!*
    *   `Wizard Idle` или `Standing Idle` (Классическая спокойная стойка).
    *   `Spellcaster Idle` (Стойка с подготовленными к магии руками).
*   **Магическая Атака (Magic Attack):**
    *   `Standing Spell Cast` или `Casting Spell` (Каст заклинания вперед руками).
    *   `Wizard Attack` или `Mage Attack` (Атака посохом или взмах рукой).
    *   `Standing Magic Attack` (Быстрый выброс энергии вперед).
*   **Магический Блок/Защита (Block):**
    *   `Magic Shield` (Создание перед собой магического силового купола).
    *   `Spell Block` или `Defend` (Ограждение руками от летящего снаряда).

#### 2️⃣ КЛАСС: СТРЕЛОК / ЛУЧНИК (ARCHER / BOWMAN)
Лучники должны держать в руках невидимый лук (в Unity мы прикрепим настоящий лук к их руке).
*   **Покой (Idle):**
    *   `Archer Idle` (Стойка с луком в руках).
    *   `Bow Idle` (Ожидание с натянутой или опущенной тетивой).
*   **Атака из лука (Ranged Attack):**
    *   `Standing Draw Arrow` (Доставание стрелы и натягивание тетивы).
    *   `Standing Aim Recoil` или `Bow Shoot` (Выстрел из лука и отдача назад).
    *   `Archer Rapid Shoot` (Быстрая серия выстрелов).
*   **Блок/Уклонение (Defense/Dodge):**
    *   Поскольку лучники редко блокируют удары щитом, вместо блока им идеально подходит уклонение:
    *   `Dodge` или `Dodge Left` / `Dodge Right` (Быстрый отскок в сторону).
    *   `Ducking` (Приседание/уклонение от удара по голове).

#### 3️⃣ КЛАСС: ВОИН (WARRIOR / KNIGHT)
Воины сражаются мечами, топорами и щитами.
*   **Покой (Idle):**
    *   `Warrior Idle` (Боевая стойка воина).
    *   `Sword And Shield Idle` (Стойка, когда в одной руке меч, во второй — щит).
    *   `Greatsword Idle` (Стойка с тяжелым двуручным мечом).
*   **Атака ближнего боя (Melee Attack):**
    *   `Sword Slash` (Горизонтальный или диагональный удар мечом).
    *   `Greatsword Slash` (Тяжелый замах двуручным мечом сверху вниз).
    *   `Sword Strike` (Колющий удар вперед).
    *   `Combo Attack` (Связка из двух-трех ударов).
*   **Блок щитом или мечом (Block):**
    *   `Shield Block` (Воин закрывается щитом спереди, принимая удар).
    *   `Standing Block` (Удержание щита в активной защите).
    *   `Sword Block` (Блокирование удара лезвием меча).

#### 🏃‍♂️ ОБЩИЕ АНИМАЦИИ ДЛЯ ВСЕХ КЛАССОВ (Бег, урон, смерть):
*   **Бег (Run):**
    *   Ищите слово `Run` (Бег) или `Sprint` (Спринт).
    *   ⚠️ **КРИТИЧЕСКИ ВАЖНО:** При поиске анимаций бега или ходьбы обязательно поставьте галочку **`In Place`** (Бег на месте) в правой панели настроек Mixamo! Персонаж должен бежать на одном месте, как на беговой дорожке. Если этого не сделать, он убежит за пределы экрана, и импортировать такую анимацию в игру будет невозможно!
*   **Получение урона (Get Hit):**
    *   `Get Hit` (Персонаж откидывает голову или тело назад от удара).
    *   `Hit To Head` или `Body Hit` (Удар в голову / в тело).
    *   `Impact` (Резкое вздрагивание от попадания).
*   **Падение и Смерть (Fall / Death):**
    *   `Death` (Классическая смерть — падение навзничь на спину).
    *   `Die` или `Killed` (Падение лицом вперед или на бок).
    *   `Fall Backward` (Падение назад).

---

### 💾 ЭТАП 2: КАК ПРАВИЛЬНО СКАЧАТЬ АНИМАЦИЮ С MIXAMO

Когда вы выбрали нужную анимацию, нажмите на большую оранжевую кнопку **`Download`** (Скачать) в правом верхнем углу сайта.

Появится маленькое окошко настроек. Установите параметры **СТРОГО** следующим образом:

1.  **Format:** Выберите **`FBX for Unity (.fbx)`** (это специальный формат, оптимизированный под скелет и систему координат Unity).
2.  **Skin:** Выберите **`Without Skin`** (Без меша/кожи).
    *   *Почему это мега-важно:* У вас в Unity уже есть сам меш Мага (`Magic_Battles_Osnova`). Загружать меш манекена Mixamo вместе с каждой анимацией не нужно — это увеличит размер игры в 10 раз! Опция `Without Skin` скачивает только невесомый файл костей и ключевых кадров анимации.
3.  **Frames per Second (FPS):** Выберите **`30`** (этого абсолютно достаточно, Unity плавно сглаживает и интерполирует кадры на любой частоте монитора 60/120/240Гц).
4.  **Keyframe Reduction:** Выберите **`none`** (Без сокращения ключевых кадров, чтобы анимация была максимально детальной, плавной и без подергиваний).

Нажмите кнопку **`Download`** в окне настроек и сохраните файл на свой компьютер (он будет называться как-то вроде `Mage_Idle.fbx`).

---

### 📥 ЭТАП 3: ИМПОРТ ФАЙЛОВ В UNITY 6

1.  В окне **Project** внутри Unity перейдите в папку, где лежит ваш Маг: `Assets/Models/Heroes_Battles/` (для порядка можете создать там подпапку `Animations`).
2.  Перетащите скачанный с Mixamo файл (например, `Mage_Idle.fbx`) прямо из проводника Windows в эту папку в окне Project.

---

### 🛠️ ЭТАП 4: ТОНКАЯ НАСТРОЙКА АНИМАЦИИ В ИНСПЕКТОРЕ UNITY (Важнейший шаг!)

Каждую импортированную анимацию необходимо настроить в инспекторе Unity. Выберите ваш файл анимации (например, `Mage_Idle`) в окне Project и перейдите в окно **Inspector** справа.

Там вам нужно настроить две вкладки: **Rig** и **Animation**.

#### 📐 Шаг A: Настройка вкладки RIG (Связываем кости)
1.  Перейдите во вкладку **`Rig`** в самом верху инспектора.
2.  В выпадающем меню **Animation Type** выберите **`Humanoid`** (Гуманоид).
3.  В выпадающем меню **Avatar Definition** выберите **`Copy From Other Avatar`** (Скопировать из другого аватара).
    *   *Зачем это делать:* Поскольку все ваши герои имеют одинаковую структуру костей (Mixamo-скелет), им не нужны индивидуальные аватары.
4.  В появившемся поле **Source** кликните на маленький кружочек справа и выберите в списке аватар вашего основного Мага: **`Magic_Battles_Osnova-Avatar`** (или перетащите его мышкой из проекта).
5.  Нажмите кнопку **`Apply`** в правом нижнем углу!
    *   *Результат:* Теперь Unity знает, что эта анимация должна воспроизводиться на костях вашего Мага идеально «один в один».

#### 🎬 Шаг B: Настройка вкладки ANIMATION (Параметры воспроизведения)
Перейдите на вкладку **`Animation`** вверху инспектора. Прокрутите настройки вниз до следующих важнейших пунктов:

1.  **Настройка циклов (Looping):**
    *   Для анимаций **Покоя (Idle), Бега (Run), Ходьбы (Walk)** — обязательно поставьте галочку **`Loop Time`** (Зациклить воспроизведение). Рядом с ней загорится галочка **`Loop Pose`** (для сглаживания перехода конца в начало).
    *   Для анимаций **Атаки (Attack), Урона (Get Hit) и Смерти (Death)** — эту галочку ставить **НЕ НАДО**! Они должны проиграться ровно один раз и остановиться.

2.  **Запекание вращения и позиции (Bake Into Pose) — Спасает от скольжения и сдвигов!**
    Найдите раздел **Root Transform Rotation** и **Root Transform Position**. Настройте их, чтобы модель не улетала со своего логического центра:

    *   **Root Transform Rotation (Поворот головы/тела):**
        *   Поставьте галочку **`Bake Into Pose`** (Запечь в позу).
        *   **Based Upon:** Установите **`Original`** (или `Body Orientation`).
        *   *Что это дает:* Наш маг не будет самопроизвольно разворачиваться во время атаки или бега.
    *   **Root Transform Position (Y) (Высота над землей):**
        *   Поставьте галочку **`Bake Into Pose`** (Запечь в позу).
        *   **Based Upon:** Установите **`Original`** (или `Feet` / ступни).
        *   *Что это дает:* Модель Мага никогда не провалится под текстуру пола и не взлетит вверх во время анимации.
    *   **Root Transform Position (XZ) (Смещение вперед-назад и влево-вправо):**
        *   Поставьте галочку **`Bake Into Pose`** (Запечь в позу).
        *   **Based Upon:** Установите **`Original`** (или `Center of Mass`).
        *   *Что это дает:* Исключает «дрифт» и скольжение персонажа. Его физический коллайдер и визуальная модель всегда будут находиться в одной точке!

3.  В самом низу инспектора нажмите кнопку **`Apply`**!

---

### 🎮 ЭТАП 5: КАК ЗАПУСТИТЬ АНИМАЦИЮ НА ПЕРСОНАЖЕ

Теперь, когда у вас есть настроенная анимация, вы можете легко проверить её работу:

1.  Выделите Мага на сцене или откройте его в режиме Prefab.
2.  В окне **Project** раскройте ваш файл анимации (нажмите на маленькую стрелочку справа от иконки `Mage_Idle.fbx`).
3.  Внутри вы увидите скрытый ассет анимации с синей иконкой треугольника (обычно называется `mixamo.com`).
4.  Просто перетащите этот синий треугольник мышкой прямо на объект Мага в окне **Hierarchy** или на сцене!
5.  Unity автоматически создаст компонент **Animator** на маге и создаст файл **Animator Controller**, который свяжет нашего персонажа с анимацией покоя!
6.  Нажмите кнопку **Play** в Unity — и ваш Маг оживет: его борода зашевелится, полы мантии начнут плавно колыхаться, и он встанет в реалистичную, живую позу!
...
Ниже приведено подробнейшее пошаговое руководство по созданию оружия, его прикреплению в Blender / Unity и настройке анимаций!

---

### 🎨 ШАГ 1. ГЕНЕРАЦИЯ ОРУЖИЯ В LEONARDO.AI (2D-ИЗОБРАЖЕНИЕ БЕЗ ФОНА)

Для того чтобы нейросеть **Tencent Hunyuan3D** создала качественную 3D-модель, ей нужно подать на вход идеально чистое 2D-изображение оружия: строго сбоку или спереди, на плоском белом фоне, без теней, бликов и лишних объектов.
...
##### 3. 🔮 Оружие для Мага (Мистический посох с кристаллом):
> **Prompt:** `An ancient wizard magic staff, gnarled dark oak wood shaft, glowing purple arcane crystal locked in a wooden claw at the top, isolated on a solid plain white background, flat 2D game asset icon, front view, orthographic projection, no shadows, sharp details, high contrast`

---

### 🌀 ШАГ 2. СОЗДАНИЕ 3D-МОДЕЛИ В TENCENT HUNYUAN3D

**Tencent Hunyuan3D** — это передовая нейросеть для генерации 3D-моделей по одной картинке (Image-to-3D). Она идеально подходит для создания игрового оружия.

1.  Перейдите на официальный сайт: [https://3d.hunyuan.tencent.com](https://3d.hunyuan.tencent.com) (или используйте их официальный Space на Hugging Face).
2.  Загрузите полученную из Leonardo.ai картинку (например, изображение меча на белом фоне) в окно загрузки изображения.
3.  Нажмите кнопку **Generate** (Сгенерировать). Нейросеть создаст 3D-модель (это займет около 1–2 минут).
4.  Проверьте модель во встроенном 3D-вьювере на сайте.
5.  Нажмите кнопку **Download** (Скачать) и выберите формат **`.gltf`** или **`.obj`** (формат GLTF предпочтительнее, так как он сохраняет правильные текстуры и материалы).

---

### 📐 ШАГ 3. ПОДГОТОВКА И КУПИРОВАНИЕ ОРУЖИЯ В BLENDER (Выравнивание Pivot Point)

Когда вы импортируете сгенерированное оружие, у него будет случайный центр вращения (**Pivot Point**). Если не исправить его в Blender, то при прикреплении к руке персонажа меч или посох будут вращаться вокруг воздуха, а не рукоятки.

1.  Запустите **Blender** и импортируйте оружие: `File ➔ Import ➔ glTF 2.0 (.gltf/.glb)` или `.obj`.
2.  Перейдите в режим редактирования (`Edit Mode`, кнопка `Tab`).
3.  Выделите всё оружие (клавиша `A`), затем переместите (`G`), поверните (`R`) и масштабируйте (`S`) модель так, чтобы **рукоятка оружия (то место, где персонаж будет держать его ладонью) находилась точно в центре координат сцены (в точке 0,0,0)**.
    *   *Зачем это делать:* Точка `0,0,0` станет опорным центром (Pivot Point) вашего оружия. При импорте в Unity оно сядет в руку персонажа идеально ровно.
4.  Убедитесь, что оружие ориентировано правильно: лезвие меча или вершина посоха должны смотреть строго вверх вдоль оси **Y** (или оси **Z** в зависимости от настроек экспорта).
5.  Вернитесь в `Object Mode` (`Tab`).
6.  Сбросьте все трансформации: Нажмите `Ctrl + A` ➔ выберите **`All Transforms`** (Все трансформации). Центр объекта переместится точно в рукоятку.
7.  Экспортируйте готовое оружие: `File ➔ Export ➔ FBX (.fbx)`. Назовите файлы понятно: `Weapon_Sword.fbx`, `Weapon_Bow.fbx`, `Weapon_Staff.fbx`.

---

### 🔗 ШАГ 4. ПРИКРЕПЛЕНИЕ ОРУЖИЯ К ПЕРСОНАЖАМ (Два лучших способа)

У вас есть два пути крепления оружия: **Способ А (Внутри Unity — Рекомендуемый для игр)** и **Способ Б (Внутри Blender — Для запекания в единый файл)**. 

---

#### 📌 СПОСОБ А: Использование Sockets (Розеток-якорей) прямо в Unity 6 (РЕКОМЕНДУЕТСЯ)
*Этот способ намного лучше, так как позволяет динамически менять оружие (например, если игрок покупает новый меч в магазине лавки замка).*

Каждый гуманоидный FBX-персонаж после импорта в Unity имеет детальную иерархию костей в окне **Hierarchy**. Нам нужно найти кость ладони и прикрепить оружие туда.

1.  Перетащите файлы вашего оружия (`Weapon_Sword.fbx`, `Weapon_Bow.fbx`, `Weapon_Staff.fbx`) в Unity в окно **Project** (в папку `Assets/Models/Heroes_Battles/`).
2.  Перетащите вашего персонажа (например, Воина `Voin_Battles_Osnova`) на сцену или откройте его Prefab.
3.  В окне **Hierarchy** раскройте структуру костей Воина по следующему пути (у разных моделей названия костей могут немного отличаться, но логика всегда одна):
    *   `Voin_Battles_Osnova` ➔ `Root` ➔ `Hips` ➔ `Spine` ➔ `Spine1` ➔ `Spine2` ➔ `RightShoulder` ➔ `RightArm` ➔ `RightForeArm` ➔ **`RightHand`** (Правая ладонь).
4.  Кликните правой кнопкой мыши по кости **`RightHand`** ➔ выберите **`Create Empty`** (Создать пустой объект).
5.  Переименуйте созданный пустой объект в **`Sword_Socket`** (или `Weapon_Holder`).
6.  Перетащите файл оружия `Weapon_Sword` из окна **Project** и сделайте его дочерним объектом для `Sword_Socket` (перетащив его прямо на него в иерархии).
7.  Сбросьте координаты оружия: В окне **Inspector** у объекта `Weapon_Sword` установите **Position** в `(0, 0, 0)` и **Rotation** в `(0, 0, 0)`.
8.  Теперь вручную с помощью инструментов перемещения (`W`) и вращения (`E`) в Unity настройте положение `Sword_Socket` так, чтобы рукоятка меча легла точно в кулак Воина.
9.  **Готово!** Поскольку объект оружия привязан к кости ладони `RightHand`, при воспроизведении абсолютно любой анимации (бега, атаки, покоя) меч будет идеально двигаться вместе с рукой Воина!
10. Для **Стрелка (Strelok)** проделайте то же самое, но прикрепите лук к **`LeftHand`** (Левой руке), чтобы в правой руке он мог держать стрелу при натяжении тетивы!

---

#### 📌 СПОСОБ Б: Прикрепление оружия через кости (Bone Parenting) в Blender
*Если вы хотите, чтобы оружие было намертво вшито в скелет персонажа еще до экспорта в Unity.*

1.  Откройте ваш `.blend` файл с персонажем (например, с Воином, у которого уже настроен скелет `Armature`).
2.  Импортируйте оружие: `File ➔ Import ➔ FBX` (выберите `Weapon_Sword.fbx`).
3.  В `Object Mode` переместите и поверните меч так, чтобы его рукоятка лежала ровно в правой руке персонажа.
4.  Сначала выделите **Меч** кликом левой кнопки мыши.
5.  Затем, зажав клавишу **`Shift`**, кликните по **Скелету персонажа (Armature)**.
6.  Переключитесь в режим позы: В левом верхнем углу вместо `Object Mode` выберите **`Pose Mode`** (Режим позы).
7.  Кликните по кости правой ладони (она будет называться как-то вроде `mixamorig:RightHand` или `Hand.R`). Кость подсветится голубым/бирюзовым цветом.
8.  Нажмите комбинацию клавиш **`Ctrl + P`** и в появившемся меню выберите пункт **`Bone`** (Кость).
9.  **Готово!** Теперь меч жестко прилинкован к кости руки. Перейдите в `Object Mode`, выделите кости, поверните руку — вы увидите, что меч идеально следует за костями! При экспорте этой модели в Unity оружие экспортируется вместе с ней.

---

### 🏃 ШАГ 5. СКАЧИВАНИЕ ПРАВИЛЬНЫХ ВООРУЖЕННЫХ АНИМАЦИЙ С MIXAMO (ПО ШАГАМ)

Обычные анимации без оружия (например, простой бег с расслабленными руками) будут выглядеть нелепо: оружие будет проходить сквозь тело персонажа. Для этого на Mixamo нужно скачать специальные анимации, созданные под оружие.

#### ⚔️ Поиск анимаций для Воина (Sword & Shield / Greatsword):
1.  Перейдите на сайт [Mixamo.com](https://mixamo.com) и войдите в аккаунт.
2.  Загрузите вашего Воина без оружия как временного персонажа через кнопку **Upload Character** (если он еще не загружен), чтобы видеть предпросмотр анимаций прямо на нем.
3.  В строке поиска (Search) введите ключевые слова:
    *   `sword idle` — Поза покоя с мечом (персонаж держит меч наготове).
    *   `sword attack` / `slash` — Удары мечом сверху, сбоку, колющие удары.
    *   `sword run` / `sword walk` — Бег/ходьба в боевой стойке с мечом в руке.
    *   `sword block` — Стойка защиты/парирования удара.
4.  Выберите наиболее плавную и подходящую анимацию.

#### 🏹 Поиск анимаций для Стрелка (Bow & Arrow):
1.  Загрузите в Mixamo модель Стрелка (`Strelok_Battles_Osnova`).
2.  В строке поиска введите:
    *   `standing draw arrow` / `standing draw bow` — Натяжение тетивы и выстрел из лука.
    *   `bow idle` — Стойка покоя с луком в левой руке.
    *   `bow run` / `bow walk` — Движение с луком наготове.
3.  Выберите анимации, где руки персонажа расположены так, будто они сжимают лук и тянут стрелу.

#### 🔮 Поиск анимаций для Мага (Magic Staff):
1.  Загрузите в Mixamo модель Мага (`Magic_Battles_Osnova`).
2.  В строке поиска введите:
    *   `standing spell cast` / `magic spell` — Применение заклинания обеими руками или посохом.
    *   `staff idle` / `wizard idle` — Поза покоя мага, опирающегося на посох.
    *   `staff run` — Бег мага, удерживающего посох в руке.

#### 📥 Правильные параметры скачивания анимаций с Mixamo:
Когда вы выбрали нужную анимацию, нажмите кнопку **`Download`** в правом верхнем углу и укажите следующие настройки:
*   **Format (Формат):** `FBX for Unity (.fbx)`
*   **Skin (Оболочка):** Выберите **`Without Skin`** (Без оболочки).
    *   *Почему это важно:* Вы уже импортировали тяжелую 3D-модель персонажа с текстурами в Unity на предыдущем шаге. Файлы анимаций должны содержать только информацию о движении костей, без дублирования меша. Это сэкономит десятки мегабайт оперативной памяти вашего проекта!
*   **Frames per Second (Кадры в секунду):** `30` (Оптимально для производительности) или `60` (Для максимальной плавности).
*   **Keyframe Reduction (Сокращение ключей):** `none` (Без сокращения).

Нажмите **`Download`** и сохраните файл. Рекомендуется сразу переименовать файлы на компьютере (например, `Voin_Sword_Idle.fbx`, `Strelok_Bow_Attack.fbx`, `Mage_Staff_Run.fbx`), чтобы не запутаться!

---

### 🎬 ШАГ 6. ОКОНЧАТЕЛЬНАЯ НАСТРОЙКА И ЗАПУСК В UNITY 6

1.  Перетащите скачанные файлы анимаций (`Without Skin`) в папку `Assets/Models/Heroes_Battles/Animations/` в Unity.
2.  Для каждого файла анимации настройте вкладку **`Rig`** в инспекторе Unity:
    *   **Animation Type:** `Humanoid`
    *   **Avatar Definition:** `Copy From Other Avatar`
    *   **Source:** Выберите аватар соответствующего персонажа (например, `Voin_Battles_Osnova-Avatar` для анимаций воина).
...
3.  Настройте вкладку **`Animation`**:
    *   Для Idle, Run, Walk поставьте галочку **`Loop Time`** и **`Loop Pose`**.
    *   Для Attack, Get Hit, Death галочку **`Loop Time`** ставить **НЕ НУЖНО**.
    *   В разделах **Root Transform Rotation**, **Root Transform Position (Y)** и **Root Transform Position (XZ)** поставьте галочки **`Bake Into Pose`** и установите **Based Upon** в **`Original`**.
    *   Нажмите **`Apply`**.
4.  В окне **Project** нажмите правой кнопкой мыши ➔ `Create ➔ Animator Controller`. Назовите его, например, `Warrior_Controller`.
5.  Откройте его двойным кликом (откроется окно Animator). Перетащите туда синие треугольники анимаций покоя (`Voin_Sword_Idle`), бега (`Voin_Sword_Run`) и атаки (`Voin_Sword_Attack`).
6.  Создайте переходы (Transitions) между ними на основе параметров типа `Float` (`Speed`) или `Trigger` (`Attack`).
7.  Выделите вашего персонажа на сцене и перетащите созданный `Warrior_Controller` в слот **Controller** компонента **Animator**.
8.  Запустите игру! Ваш воин будет плавно дышать в боевой стойке с мечом в руке, стрелок натянет лук, а маг эффектно взмахнет светящимся посохом при сотворении заклинания!
