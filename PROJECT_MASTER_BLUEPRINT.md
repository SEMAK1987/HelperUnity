...
**Проблема:** Ошибки Unicode (\u7B80, \u4F53, \uD55C) и "квадраты" вместо букв (Китайский, Корейский). Статический шрифт не содержит этих символов.

**Пошаговое решение (Сделать шрифт Dynamic):**
1. **Создайте Font Asset:** Найдите оригинал шрифта в папке `Шрифты` (файл .ttf). ПКМ -> **Create -> TextMeshPro -> Font Asset**.
2. **Dynamic Mode:** Выберите созданный ассет (иконка [F]). В Inspector смените **Atlas Population Mode** на **Dynamic**. Нажмите **Apply**.
3. **Настройка Fallback:** Найдите основной шрифт `LiberationSans SDF`. В его списке **Fallback Font Assets** замените старый шрифт на этот новый **Dynamic** ассет.

### ↔️ ИСПРАВЛЕНИЕ СЛИПАНИЯ И ВЫЛЕТАНИЯ ТЕКСТА (Overlap Fix)
**Пошаговое решение:**
1. **Auto Size:** У текстового объекта в Inspector включите **Auto Size** и поставьте **Min Size: 18**.
2. **Spacing:** В **Extra Settings** -> **Spacing Options** увеличьте **Character** до **10-15**.
3. **Рамка:** Если текст всё еще не влезает (например, "Português"), растяните рамку текста инструментом **Rect Tool** (клавиша `T`).

### 🚀 ПРАВИЛЬНЫЙ ЗАПУСК (Без Hub и лагов)
**Бат-файл должен указывать на Unity.exe (Editor), а не на Hub:**
```batch
@echo off
echo Starting Unity 6 Engine Directly...
:: УКАЖИТЕ ПУТЬ К ВАШЕМУ Unity.exe
...
- **v18.4.9:** Ultimate Stability Sync. Bloom guidance integration. Full version parity.
- **v18.4.8:** Final Stability Sync. Bloom guidance integration. Full version parity.
