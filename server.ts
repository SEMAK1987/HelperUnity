...
async function startServer() {
  ...
  app.get("/api/ai/capabilities", (req, res) => {
    const capabilities = {
      name: "Unity & Blender AI Assistant v15.20.0 (PRO Edition)",
      description: "Ваш ультимативный ИИ-компаньон для геймдева. Спроектирован для решения задач любой сложности — от написания скриптов до создания живых миров. Теперь с поддержкой Neural Sync 2.0, К-отладки, мифических модулей и базой знаний из 3800+ экспертных видео-уроков. Поддерживает работу в трех архитектурных режимах для максимальной гибкости.",
      core_functions: [
        {
          title: "Online Mode (Gemini 1.5 Pro SSS+ Neural)",
          desc: "Максимальный интеллект уровня SSS+. Прямая нейронная связь с облачными кластерами Google и Galactic Network. Анализ архитектуры, генерация сложнейшего кода, предсказание трендов и работа с терабайтными датасетами. Использует технологию Quantum Beam и доступ к Galactic Network. Работает со всеми видео-файлами, скриптами и ссылками в реальном времени."
        },
        {
        ...
        {
          title: "No-Internet Core (Knowledge DB v4)",
          desc: "Режим 'Черный Ящик'. Мгновенный доступ к 3800+ видео, тысячам скриптов и API справочникам Unity/Blender без интернета. Поиск через бинарные индексы SSD. Ответы приходят моментально. Содержит полную копию всех обучающих данных."
        },
        {
