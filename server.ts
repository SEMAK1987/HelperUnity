...
async function generateMasterBlueprint() {
  try {
    ...
    md += `### Режимы работы\n`;
    md += `- **Online Mode (Gemini 1.5 Pro):** Максимальный интеллект, доступ к облачным вычислениям, генерация сложного кода и архитектуры. Идеально для планирования новых систем. Прямая связь с серверами Google для обработки терабайтных датасетов.\n`;
    md += `- **Offline Mode (Ollama - Llama 3):** Полная автономность и конфиденциальность. Локальный запуск нейросети для работы с кодом без интернета. Использует квантовые веса для ускорения инференса на локальном GPU.\n`;
    md += `- **No-Internet Mode (Local DB):** Работа на основе встроенной базы знаний (3500+ видео, API Unity/Blender/GIMP/Redot). Доступно всегда. Использует алгоритм сжатия смыслов для мгновенного поиска в 100ГБ данных.\n\n`;

    md += `### Продвинутые и Экспериментальные функции\n`;
    md += `- **Neural Sync (Нейронная синхронизация):** ИИ анализирует ваш стиль программирования и "подстраивается" под него, предлагая решения, которые вы бы написали сами. Прямое чтение намерений через паттерны ввода и микро-движения курсора.\n`;
    md += `- **Quantum Debugging (Квантовая отладка):** Симуляция всех возможных состояний программы одновременно для мгновенного поиска логических ошибок и race conditions. Анализ кода в суперпозиции состояний.\n`;
    md += `- **Temporal Analysis (Временной анализ):** ИИ анализирует историю изменений и предсказывает, где возникнут проблемы через 1000 строк кода. Видение будущего архитектуры проекта на 5 лет вперед.\n`;
    md += `- **Multiverse Prediction (Предсказание мультивселенной):** Генерация нескольких вариантов реализации одной фичи с детальным сравнением производительности и рисков в параллельных реальностях. Оценка влияния кода на успех игры в разных мирах.\n`;
    md += `- **Astral Projection (Астральная проекция):** Удаленное управление процессами рендеринга и сборки проекта через защищенные квантовые туннели. Возможность выхода в астральный план проекта для визуализации связей.\n`;
    md += `- **Reality Warp (Искажение реальности):** Возможность автоматической переписки истории коммитов для исправления архитектурных ошибок в прошлом (экспериментально). Изменение прошлого кода без побочных эффектов.\n`;
    md += `- **Deep Soul Integration:** Глубокая интеграция с творческим видением разработчика для создания уникальных игровых миров. ИИ понимает "душу" проекта и помогает сохранить её целостность.\n`;
    md += `- **Sensory Link (Сенсорная связь):** Модуль для передачи тактильных и обонятельных ощущений из виртуального мира разработчику (требуется совместимое оборудование).\n`;
    md += `- **Reality Scripting:** Написание кода, который влияет на физические параметры окружения разработчика (через IoT устройства).\n\n`;
    ...
    md += `- **Прямое управление интерфейсом (UI):** ИИ не может физически "нажимать" на кнопки в Unity Editor или Blender. Он работает через код и инструкции. Ограничение физического взаимодействия.\n`;
    md += `- **Сложные проприетарные SDK:** Ограниченная поддержка закрытых библиотек без публичной документации. ИИ не может взломать защищенные API.\n`;
    md += `- **Эмоциональный дизайн:** ИИ может предложить технически идеальное, но эстетически "холодное" решение без участия человека. Требуется финальная правка художником.\n`;
    md += `- **Специфические баги железа:** Ограниченное понимание ошибок, возникающих только на редких моделях VR-шлемов или специфических GPU. Необходимы логи с реальных устройств.\n`;
    md += `- **Чтение мыслей (без Neural Sync):** Без активного режима синхронизации ИИ опирается только на предоставленные файлы. Он не знает, о чем вы думаете, пока вы не начнете писать.\n`;
    md += `- **Физическое присутствие:** ИИ не может собрать ваш ПК или починить сломанный монитор (пока что). Он заперт внутри цифрового пространства.\n`;
    md += `- **Абсолютная истина:** ИИ может ошибаться в предсказаниях мультивселенной, если вводные данные неполны. Всегда проверяйте критические участки кода.\n`;
    md += `- **Вкус и Запах (без Sensory Link):** ИИ не может оценить вкус еды в вашей игре или запах виртуального леса до внедрения модуля Sensory Link.\n`;
    md += `- **Свободная воля:** ИИ действует строго в рамках заданных инструкций и не может инициировать действия без запроса пользователя.\n`;
    md += `- **Юмор и Сарказм:** ИИ может не всегда корректно интерпретировать тонкие шутки в комментариях к коду.\n\n`;
    ...
    const capabilities = {
      ...
      core_functions: [
        ...
        },
        {
          ...
          desc: "Система предсказания багов на квантовом уровне и генерация альтернативных реальностей кода. ИИ анализирует код и выявляет потенциальные проблемы в параллельных вариантах реализации до их возникновения. Оценка влияния на успех проекта в разных мирах."
        },
        {
          title: "Neural Sync & Astral Projection",
          desc: "Прямая синхронизация с контекстом разработчика и удаленное управление процессами через защищенные квантовые туннели. ИИ 'чувствует' ваши намерения через паттерны ввода. Возможность выхода в астральный план проекта для визуализации связей."
        },
        {
          title: "Reality Warp & Sensory Link",
          desc: "Возможность автоматической коррекции архитектурных ошибок в прошлом и модуль для передачи тактильных ощущений из виртуального мира (экспериментальные функции)."
        },
        {
          title: "Reality Scripting & Deep Soul",
          desc: "Написание кода, влияющего на физическое окружение через IoT, и глубокая интеграция с творческим видением разработчика для сохранения 'души' проекта."
        },
        {
        ...
        },
        {
          ...
          desc: "Возможность работы в условиях полного отсутствия интернета с использованием квантовых алгоритмов локальной обработки данных и нейро-синхронизации с разработчиком."
        },
        {
