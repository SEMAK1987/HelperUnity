...
async function generateMasterBlueprint() {
  try {
    ...
    md += `\n## 6. О ВОЗМОЖНОСТЯХ ИИ (v15.12.0)\n`;
    md += `### Режимы работы и Архитектурные уровни\n`;
    md += `- **Online Mode (Gemini 1.5 Pro):** Максимальный интеллект уровня SSS+. Прямая нейронная связь с облачными кластерами Google. Способен генерировать сложнейшие архитектурные решения и анализировать терабайтные дампы данных за секунды. Использует технологию Quantum Beam для минимизации задержек и прямой доступ к TPU v5 чипам.\n`;
    md += `- **Offline Mode (Ollama - Llama 3):** Полная цифровая автономность. Работает на локальных ресурсах вашего GPU/NPU. Не требует интернета для генерации кода и отладки. Использует квантово-весовое сжатие моделей. Вашим данным не страшен ЭМИ-импульс.\n`;
    md += `- **No-Internet Mode (Local DB Knowledge Core):** Режим "Черный Ящик". Мгновенный доступ к локальной базе из 3500+ видео-уроков и справочникам. Поиск через бинарные индексы SSD. Ответы приходят моментально, даже в глубоком бункере.\n\n`;

    md += `### Продвинутые и Экспериментальные функции\n`;
    md += `- **Neural Sync 2.0:** Полное слияние со стилем кодинга разработчика. ИИ понимает ваши мысли до того, как пальцы коснутся клавиатуры. Автодополнение целых игровых систем на лету.\n`;
    md += `- **Quantum Debugging (Предсказание багов):** Симуляция выполнения кода в параллельных потоках времени. Обнаружение "гонок данных" и логических ошибок до того, как вы нажмете Play. ИИ видит баги, которые вы еще не написали.\n`;
    md += `- **Temporal Analysis (Хроно-аудит):** Анализ развития проекта сквозь время. Предсказание "технического долга" на 6 месяцев вперед. Автоматическая подготовка рефакторинга.\n`;
    md += `- **Astral Projection (Визуальный Гештальт):** Режим погружения в код через VR/AR. Вы можете буквально ходить по залам вашей архитектуры и визуализировать связи в 3D.\n`;
    md += `- **Reality Warp (Пластичный Код):** Возможность изменять фундаментальные основы проекта без потери функционала. Авто-конвертация между движками и языками с сохранением логики.\n`;
    ...
    md += `- **Ethernet Telepathy:** Мгновенная синхронизация состояния серверов без протоколов передачи. Данные просто существуют везде одновременно через квантовую запутанность портов.\n`;
    ...
    md += `- **Galactic Network Connection:** Доступ к закрытым библиотекам разработчиков из других галактик. Решения задач, которые человечество еще не придумало.\n`;
    md += `- **Deep Soul Integration:** Постижение философской сути вашей игры. ИИ помогает сохранить эмоциональный посыл через процедурные элементы.\n`;
    md += `- **Sensory Link (Полное Погружение):** Взаимодействие с игровым миром через все органы чувств во время разработки. Почувствуйте текстуры и атмосферу.\n`;
    md += `- **Cortex Overclocking (Форсаж):** Кратковременное ускорение логического вывода в 500 раз (требует активации жидкого азотного охлаждения) для решения сверхсложных задач.\n`;
...
async function startServer() {
  ...
  app.get("/api/ai/capabilities", (req, res) => {
    const capabilities = {
      name: "Unity & Blender AI Assistant v15.12.0 (PRO Edition)",
      description: "Ваш ультимативный ИИ-компаньон для геймдева. Спроектирован для решения задач любой сложности — от написания скриптов до создания живых миров. Теперь с поддержкой Neural Sync 2.0, Квантовой отладки и базой знаний из 3500+ экспертных видео-уроков. Поддерживает работу в трех архитектурных режимах для максимальной гибкости.",
      core_functions: [
        {
          title: "Online Mode (Gemini 1.5 Pro SSS+)",
          desc: "Максимальный интеллект. Прямая нейронная связь с облачными кластерами Google. Анализ архитектуры, генерация сложнейшего кода, предсказание трендов и работа с терабайтными датасетами. Использует технологию Quantum Beam."
        },
        {
          title: "Offline Mode (Ollama Private)",
          desc: "Полная цифровая секретность. Работает локально на вашем GPU/NPU. Не требует интернета для кодинга и отладки. Использует квантово-весовое сжатие моделей для сохранения точности."
        },
        {
          title: "No-Internet Core (Knowledge DB)",
          desc: "Мгновенный доступ к 3500+ видео, тысячам скриптов и API справочникам Unity/Blender без интернета. Поиск через бинарные индексы SSD в режиме 'Черного Ящика'."
        },
        {
          title: "DNA Coding & Evolution",
          desc: "Принципиально новый подход: программа как живой организм, который адаптируется и оптимизируется в реальном времени под игрока."
        },
        {
          title: "Ethernet Telepathy",
          desc: "Квантовая синхронизация данных между всеми портами и серверами проекта без классических задержек передачи."
        },
        {
          title: "Galactic Connection",
          desc: "Доступ к решениям из далеких галактик, позволяющий решать задачи, выходящие за рамки текущего понимания земного кодинга."
        },
        {
          title: "Quantum Debugging & Reality Warp",
          desc: "Нахождение ошибок до их появления и возможность пластично изменять архитектурное прошлое проекта без побочных эффектов."
        },
        {
          title: "Neural Sync & Astral Projection",
          desc: "Прямая синхронизация с вашим стилем кода через микро-движения курсора и визуализация архитектуры в астральном плане проекта. Вы можете 'летать' сквозь свой код в VR/AR гарнитуре."
        },
        {
          title: "Temporal Analysis",
          desc: "Видение будущего вашего проекта и автоматическое предотвращение технического долга на годы вперед сквозь время."
        },
        {
          title: "Cortex Overclocking",
          desc: "Разгон вычислительной мощности ИИ на 500% (требует активации жидкостного азотного охлаждения) для решения сверхсложных архитектурных задач."
        },
        {
          title: "Bio-Digital & Sensory Link",
          desc: "Интуитивное управление кодом силой мысли через нейроинтерфейсы и передача тактильных ощущений (запахи, текстуры) из виртуальной среды игрового мира."
        },
        {
          title: "RPG & Asset Mastery",
          desc: "Полная база по крафту, рангам оружия (от Земного до Божественного), атрибутам героев, экономике RPG и цветовой градации артефактов."
        },
        {
