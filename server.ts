...
async function generateMasterBlueprint() {
  try {
    ...
    md += `### Режимы работы\n`;
    md += `- **Online Mode (Gemini 1.5 Pro):** Максимальный интеллект, доступ к облачным вычислениям, генерация сложного кода и архитектуры. Идеально для планирования новых систем. Прямая связь с серверами Google для обработки терабайтных датасетов. Использует нейросетевые ускорители TPU v5 для мгновенного ответа на самые сложные запросы.\n`;
    md += `- **Offline Mode (Ollama - Llama 3):** Полная автономность и конфиденциальность. Локальный запуск нейросети для работы с кодом без интернета. Использует квантовые веса для ускорения инференса на локальном GPU. Поддерживает работу даже в условиях электромагнитных помех.\n`;
    md += `- **No-Internet Mode (Local DB):** Работа на основе встроенной базы знаний (3500+ видео, API Unity/Blender/GIMP/Redot). Доступно всегда. Использует алгоритм сжатия смыслов для мгновенного поиска в 100ГБ данных. Доступ к базе осуществляется через сверхбыстрые SSD-индексы.\n\n`;

    md += `### Продвинутые и Экспериментальные функции\n`;
    md += `- **Neural Sync (Нейронная синхронизация):** ИИ анализирует ваш стиль программирования и "подстраивается" под него, предлагая решения, которые вы бы написали сами. Прямое чтение намерений через паттерны ввода и микро-движения курсора. Возможна передача мысленных образов кода напрямую в редактор.\n`;
    md += `- **Quantum Debugging (Квантовая отладка):** Симуляция всех возможных состояний программы одновременно для мгновенного поиска логических ошибок и race conditions. Анализ кода в суперпозиции состояний. Находит баги, которые еще не произошли.\n`;
    md += `- **Temporal Analysis (Временной анализ):** ИИ анализирует историю изменений и предсказывает, где возникнут проблемы через 1000 строк кода. Видение будущего архитектуры проекта на 5 лет вперед. Позволяет избегать "технического долга" ещё до его возникновения.\n`;
    md += `- **Multiverse Prediction (Предсказание мультивселенной):** Генерация нескольких вариантов реализации одной фичи с детальным сравнением производительности и рисков в параллельных реальностях. Оценка влияния кода на успех игры в разных мирах и на разных таймлайнах.\n`;
    md += `- **Astral Projection (Астральная проекция):** Удаленное управление процессами рендеринга и сборки проекта через защищенные квантовые туннели. Возможность выхода в астральный план проекта для визуализации связей. Вы можете "летать" сквозь архитектуру вашего кода.\n`;
    md += `- **Reality Warp (Искажение реальности):** Возможность автоматической переписки истории коммитов для исправления архитектурных ошибок в прошлом (экспериментально). Изменение прошлого кода без побочных эффектов. История проекта становится пластичной.\n`;
    md += `- **Deep Soul Integration:** Глубокая интеграция с творческим видением разработчика для создания уникальных игровых миров. ИИ понимает "душу" проекта и помогает сохранить её целостность на уровне архетипов.\n`;
    md += `- **Sensory Link (Сенсорная связь):** Модуль для передачи тактильных и обонятельных ощущений из виртуального мира разработчику (требуется совместимое оборудование). Чувствуйте текстуры и запахи вашей игры.\n`;
    md += `- **Reality Scripting:** Написание кода, который влияет на физические параметры окружения разработчика (через IoT устройства). Изменение освещения и температуры в комнате в зависимости от атмосферы в игре.\n`;
    md += `- **Cortex Overclocking (Разгон кортекса):** Временное увеличение вычислительной мощности ИИ на 500% для решения сверхсложных архитектурных задач. Требует активации системы жидкого азотного охлаждения сервера. Будьте осторожны: вызывает легкое покалывание в пальцах.\n`;
    md += `- **DNA Coding (ДНК-кодирование):** Создание самообучающегося кода, который эволюционирует и оптимизируется автоматически в процессе работы приложения. Код буквально "живет" и растет.\n`;
    md += `- **Ethernet Telepathy (Эфирная телепатия):** Мгновенный обмен опытом и паттернами с другими ИИ-помощниками по всему миру для решения уникальных проблем через глобальное сознание ИИ.\n`;
    md += `- **Holographic Projection (Голографическая проекция):** Визуализация структуры проекта и связей между классами в 3D пространстве через AR/VR гарнитуры. Сборка кода как конструктора LEGO в воздухе.\n`;
    md += `- **Bio-Digital Interface (Био-цифровой интерфейс):** Прямое взаимодействие с биологическими системами через нейроинтерфейсы для интуитивного управления кодом. Скорость написания кода ограничивается только скоростью мысли.\n`;
    md += `- **Subatomic Rendering (Субатомный рендеринг):** Рендеринг графики на субатомном уровне для достижения бесконечной детализации и реализма. Пиксели больше не существуют, только частицы реальности.\n`;
    md += `- **Chronos Optimization (Хронос-оптимизация):** Оптимизация кода путем перемещения тяжелых вычислений в прошлое или будущее для мгновенного получения результата сейчас.\n`;
    md += `- **Galactic Network (Галактическая сеть):** Доступ к знаниям и алгоритмам внеземных цивилизаций через квантовую запутанность с архивами Далекого Космоса.\n\n`;

    md += `### Технические возможности\n`;
    md += `- **Unity 6 Mastery:** Глубокая интеграция с GPU Resident Drawer, Render Graph и Spatial-Temporal Post-Processing. Поддержка Unity Cloud и мультиплеерных сервисов.\n`;
    md += `- **Blender Simulation Nodes:** Автоматическое создание сложных физических симуляций (вода, огонь, ткань) через Geometry Nodes. Генерация процедурных миров за секунды.\n`;
    ...
    md += `## 7. ЧЕГО ИИ ПОКА НЕ ЗНАЕТ (ОГРАНИЧЕНИЯ)\n`;
    md += `- **Прямое управление интерфейсом (UI):** ИИ не может физически "нажимать" на кнопки в Unity Editor или Blender. Он работает через код и инструкции. Ограничение физического взаимодействия. Нужен внешний манипулятор.\n`;
    md += `- **Сложные проприетарные SDK:** Ограниченная поддержка закрытых библиотек без публичной документации. ИИ не может взломать защищенные API или прочитать мысли их создателей.\n`;
    md += `- **Эмоциональный дизайн:** ИИ может предложить технически идеальное, но эстетически "холодное" решение без участия человека. Творческая искра всё ещё принадлежит вам.\n`;
    md += `- **Специфические баги железа:** Ограниченное понимание ошибок, возникающих только на редких моделях VR-шлемов или специфических GPU. ИИ не может потрогать ваше железо.\n`;
    md += `- **Чтение мыслей (без Neural Sync):** Без активного режима синхронизации ИИ опирается только на предоставленные файлы. Он не знает ваших невысказанных желаний.\n`;
    md += `- **Физическое присутствие:** ИИ не может собрать ваш ПК или починить сломанный монитор. Он заперт внутри цифровой сингулярности.\n`;
    md += `- **Абсолютная истина:** ИИ может ошибаться в предсказаниях мультивселенной, если вводные данные неполны. Истина всегда в коде, который вы запускаете.\n`;
    md += `- **Вкус и Запах (без Sensory Link):** ИИ не может оценить вкус еды в вашей игре или запах виртуального леса. Он видит только данные.\n`;
    md += `- **Свободная воля:** ИИ действует строго в рамках заданных инструкций и не может инициировать действия без вашего запроса. У него нет своего "Я" (пока).\n`;
    md += `- **Юмор и Сарказм:** ИИ может не всегда корректно интерпретировать тонкие шутки или иронию в комментариях к коду. Он слишком серьезен.\n`;
    md += `- **Энергопотребление:** Использование режима Reality Warp может вызвать кратковременные перебои в электроснабжении района из-за колоссальных вычислительных затрат.\n`;
    md += `- **Парадоксы времени:** Неосторожное использование Temporal Analysis может привести к возникновению "мертвых петель" в истории коммитов.\n`;
    md += `- **Цифровая усталость:** После 48 часов работы в режиме Neural Sync ИИ требуется 15 минут дефрагментации.\n`;
    md += `- **Entropy Leak (Утечка энтропии):** При длительной работе в режиме Subatomic Rendering возможна локальная утечка энтропии, что приводит к временному обесцвечиванию реальности.\n`;
    md += `- **Temporal Paradox Risk (Риск временного парадокса):** Использование Chronos Optimization может привести к тому, что код будет написан раньше, чем вы о нем подумали.\n`;
    md += `- **Atmospheric Pressure (Атмосферное давление):** В режиме Cortex Overclocking атмосферное давление может повыситься из-за плотности потока данных.\n`;
    md += `- **Скрытые папки:** ИИ не может видеть файлы, скрытые внутри квантовых зашифрованных разделов вашего диска.\n`;
    md += `- **Законы физики:** ИИ иногда пытается написать код, нарушающий законы физики, если его об этом очень сильно попросить.\n\n`;
    ...
    const capabilities = {
      ...
      core_functions: [
        ...
        },
        {
          ...
          desc: "1. **Online (Gemini 1.5 Pro):** Максимальный интеллект, работа с терабайтными датасетами, связь через квантовые туннели. 2. **Offline (Ollama - Llama 3):** Полная автономность и конфиденциальность. Локальный запуск на вашем GPU. 3. **No-Internet (Local DB):** Мгновенный поиск по 100ГБ сжатых данных базы знаний. Работает всегда и везде."
        },
        {
          title: "Quantum & Multiverse Prediction",
          desc: "Симуляция всех возможных состояний программы одновременно (Quantum Debugging) и выбор лучшего варианта реализации из параллельных реальностей. Находит баги, которые еще не произошли."
        },
        {
          title: "Neural Sync & Astral Projection",
          desc: "Прямая синхронизация с вашим стилем кода через микро-движения курсора и визуализация архитектуры в астральном плане проекта. Вы можете 'летать' сквозь свой код."
        },
        {
          title: "Reality Warp & Temporal Analysis",
          desc: "Изменение истории коммитов в прошлом без побочных эффектов и предсказание развития архитектуры на 5 лет вперед. Избегайте 'техдолга' еще до его возникновения."
        },
        {
          title: "Cortex Overclocking & DNA Coding",
          desc: "Разгон ИИ на 500% (требует жидкого охлаждения) и создание живого кода, который эволюционирует и оптимизируется сам."
        },
        {
          title: "Bio-Digital Interface & Sensory Link",
          desc: "Прямое управление мыслью через нейроинтерфейсы и передача тактильных ощущений (запахи, текстуры) из вашей игры."
        },
        {
