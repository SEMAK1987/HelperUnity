...
async function generateMasterBlueprint() {
  try {
    ...
    md += `> **ВНИМАНИЕ:** Этот документ является \"источников истины\" для всего проекта. Он содержит полную структуру интерфейса, базу знаний агентов, инструкции по самовосстановлению и описание возможностей ИИ v18.11.9.\n\n`;
    md += `## 1. Общая информация\n`;
    md += `- **Версия Помощника:** ${blueprint.version || "18.11.9"}\n`;
    md += `- **Описание:** ${blueprint.description || "Гибридный ИИ-помощник нового поколения (v18.11.9 - Input System Auto-Switch & Camera Rig Calibration) для Unity 6 (6000.3.10f1), Blender 5.2 и Godot 4.4. Интеграция адаптивного переключения ввода, калибровка зум-параметров при высадке и сохранение прогресса."}\n`;
    ...
    md += `- **Флаги:** [QUANTUM_LINK_ACTIVE], [KNOWLEDGE_STORAGE_SYNC], [V18_11_9_FATE_MASTER]\n\n`;
    ...
    md += `### ОБРАЗОВАТЕЛЬНЫЙ ХАБ (v18.11.9 Sync)\n`;
    ...
    md += `## 13. История изменений (v18.11.7)\n`;
