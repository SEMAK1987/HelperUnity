...
async function generateMasterBlueprint() {
  try {
    ...
    md += `- **Reality Warp (Искажение реальности):** Возможность автоматической переписки истории коммитов для исправления архитектурных ошибок в прошлом (экспериментально).\n\n`;
    ...
    md += `- **Физическое присутствие:** ИИ не может собрать ваш ПК или починить сломанный монитор (пока что).\n\n`;
    ...
    const capabilities = {
      ...
      core_functions: [
        ...
        },
        {
          ...
          desc: "Система предсказания багов на квантовом уровне и генерация альтернативных реальностей кода. ИИ анализирует код и выявляет потенциальные проблемы в параллельных вариантах реализации до их возникновения."
        },
        {
          title: "Neural Sync & Astral Projection",
          desc: "Прямая синхронизация с контекстом разработчика и удаленное управление процессами через защищенные квантовые туннели. ИИ 'чувствует' ваши намерения и адаптирует стиль кода."
        },
        {
          title: "Reality Warp (Experimental)",
          desc: "Возможность автоматической коррекции архитектурных ошибок в прошлом через интеллектуальную переписку истории проекта (экспериментальная функция)."
        },
        {
