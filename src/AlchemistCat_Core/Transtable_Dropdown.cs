...
public class Transtable_Dropdown : MonoBehaviour
{
    ...
    public void UpdateDropdown()
    {
        ...
        try
        {
            ...
            {
                // AUTO-DETECT Logic
                if (dropdown.options.Count == 9 || dropdown.options.Count == 3 || gameObject.name.Contains("Language")) // Language List
                {
                    // Для Алхимического Кота поддерживаются только 3 языка: Русский, English, Türkçe
