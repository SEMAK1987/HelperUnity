...
`
  ...
  {
    name: "Blender Auto Binder",
    filename: "blender_auto_binder.py",
    location: "Scripts/Blender/blender_auto_binder.py",
    language: "python",
    descriptionRU: "Скрипт автоматического скиннинга. Автоматически находит меш персонажа (node_0), скелет (FateHumanoid_Armature), очищает старые связи, сливает дубликаты вершин для устранения ошибок Heat Weighting и привязывает модель с автоматическими весами.",
    descriptionEN: "One-click automatic skinning script. Automatically locates mesh (node_0) and armature (FateHumanoid_Armature), clears old relationships, merges double vertices to eliminate Heat Weighting failures, and binds with automatic weights.",
    code: `import bpy
...
`
  ...
  {
    name: "Blender Rigging Optimizer",
    filename: "BlenderRiggingOptimizer.py",
    location: "Scripts/Blender/BlenderRiggingOptimizer.py",
    language: "python",
    descriptionRU: "Скрипт оптимизации сетки персонажа. Объединяет разрозненные куски в один меш, поворачивает модель лицом по оси Z к камере, сваривает вершины и снижает вес полигонов до Mixamo-оптимального порога.",
    descriptionEN: "Rigging and mesh pre-processor. Combines separate mesh parts, auto-aligns facing orientation, welds duplicate vertices, and decimates excessive poly counts to meet Mixamo optimization constraints.",
    code: `import bpy

def optimize_character_for_mixamo():
    print("=== STARTING HERO OPTIMIZATION FOR MIXAMO ===")
    if bpy.ops.object.mode_set.poll():
        bpy.ops.object.mode_set(mode='OBJECT')
    ...
    if len(mesh_objects) > 1:
        print(f"Joining {len(mesh_objects)} objects into one...")
        bpy.ops.object.join()
    else:
        print("Model is already a single mesh.")
 
    print("Aligning character orientation...")
    active_obj.location = (0, 0, 0)
    active_obj.rotation_euler = (0, 0, 0)
    active_obj.rotation_euler[2] = 3.14159  # 180 degrees in radians to face front
    ...
    print(f"Original polygon count: {poly_count}")
    
    if poly_count > 60000:
        print("Mesh too heavy. Applying Decimate optimization...")
        ...
        print(f"Optimized polygon count: {len(active_obj.data.polygons)}")
    else:
        print("Polygon count within optimal range. Skipping Decimate.")

    print("=== OPTIMIZATION COMPLETED SUCCESSFULLY! ===")
    print("Export instructions: Export to FBX, Apply Scalings: 'FBX All', Path Mode: 'Copy' with Embed Textures active.")
